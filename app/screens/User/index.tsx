...
class User extends React.PureComponent<Props, State> {
    ...
    if (numOfPages > prevNumOfPages) {
      ...
      for (const page of _.range(prevNumOfPages, numOfPages)) {
        if (!this.pageToSocket[page]) {
          ...
          this.pageToSocket[page]!.on('$err', err => {
  ...
  render() {
    const { avatar, header, posts, bio, displayName, navigation } = this.props
    const publicKey = navigation.getParam('publicKey')

    console.warn(_.size(this.state.posts))
    console.warn(_.size(this.state.numOfPages))
    ...
    renderItem = ({ item }: ListRenderItemInfo<Common.Schema.PostBase>) => {
      return (
        <Post
          author={Common.Schema.createEmptyUser(
